...
def generate_mock_report(doc_type, comp_mode, achieve_ratio=True):
    ...
    report = {
        ...
        "bnf_compliant": "true",
        "completed_at": "2025-05-07T17:30:00Z",
...
def main():
    ...
    print(f"Correct interpretations: {results['correct']}/{results['total']} "
          f"({100 * results['correct']/results['total']:.1f}%)")
    print(f"Incorrect interpretations: {results['incorrect']}/{results['total']} "
          f"({100 * results['incorrect']/results['total']:.1f}%)")
    print(f"Failed tests: {results['failed']}/{results['total']} "
          f"({100 * results['failed']/results['total']:.1f}%)")
