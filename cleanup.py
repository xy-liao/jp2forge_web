...
def stop_running_processes():
    ...
    try:
        ...
        for line in result.stdout.strip().split('\n'):
            if ('runserver' in line or 'celery -A jp2forge_web' in line) and 'grep' not in line:
                lines.append(line)
        ...
        for line in lines:
            ...
            try:
                ...
                pid_int = int(pid)
                cmd_check = ["ps", "-p", str(pid_int), "-o", "command="]
                proc_cmd = subprocess.run(cmd_check, capture_output=True, text=True).stdout.strip()
