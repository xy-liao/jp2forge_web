...
def test_combination(doc_type, comp_mode, bnf_compliant, verbose=False, save_reports=False, force_mock=False):
    ...
    with tempfile.TemporaryDirectory() as output_dir:
        ...
        if save_reports:
            save_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "reports_test")
...
def main():
    ...
    print(f"Successful conversions: {results['successful']}/{results['total']} "
          f"({100 * results['successful']/results['total']:.1f}%)")
    print(f"Failed conversions: {results['failed']}/{results['total']} "
